/*
  ...
  I/Q receive architecture update:
    ...
    - same-band CAT tuning is click-free: CLK0/RX/USB stay live while PLLA is
      updated without a PLL soft reset; full blank/rebuild is used only when
      the fixed MS0 band profile changes
    - Kenwood MD1/MD2 select LSB/USB; CW terminal forces USB because its RX LO is carrier-10.005 kHz
    ...
    - app performs noise blanking, 300 Hz centred filtering, AGC, S-meter, waterfall and Morse decode
    ...
    - calibration corrects relative Q-channel gain and residual quadrature error,
      stores the coefficients in flash-backed EEPROM emulation, then restores the prior RX setup
  ...
  A valid calibration is saved in RP2350 flash using EEPROM emulation and survives reboot.
  ...
  During CW receive, CLK0/QSD LO is tuned 10.005 kHz below the wanted RF and
  the I/Q DSP is forced to USB. Therefore %F always means the actual on-air CW
  carrier / waterfall-centre RF, never the physical receive oscillator.  The
  phone BFO (300..800 Hz) is local DSP only and does not retune the radio.
*/
...
//
// CW dial semantics:
//   dialFrequencyHz = actual wanted RF and actual CW TX carrier frequency.
//   The radio validates dialFrequencyHz against the real 40/30/20/17 m limits.
//   During CW receive only, CLK0/QSD LO = dialFrequencyHz - 10.005 kHz and USB
//   is forced.  The wanted RF therefore appears at exactly 10.005 kHz in the
//   USB-audio spectrum.  The Android app owns BFO pitch, 300 Hz filtering,
//   noise blanking, AGC, waterfall processing, S-meter and CW decode.
// ---------------------------------------------------------------------------
static constexpr uint16_t CW_DEFAULT_WPM = 20;
static constexpr uint16_t CW_MIN_WPM = 5;
static constexpr uint16_t CW_MAX_WPM = 50;
static constexpr uint32_t CW_RX_CENTER_AUDIO_HZ = 10005UL;
...
uint32_t cwReceiveLoFrequencyHz() {
  if (dialFrequencyHz <= CW_RX_CENTER_AUDIO_HZ) return 0;
  return dialFrequencyHz - CW_RX_CENTER_AUDIO_HZ;
}
...
// 300 Hz centred CW filter, BFO pitch, waterfall, S-meter and Morse decoder.
// ---------------------------------------------------------------------------
...
// Smooth same-band RX tuning.  MS0 remains fixed, CLK0 remains enabled,
// the receive RF path remains connected, and the I/Q/Hilbert state is left
// untouched.  Only PLLA's frequency word is changed.  In particular, DO NOT
// soft-reset PLLA here: the Si5351 supports live/glitch-free PLL rate changes
// when the PLL is not reset.  Resetting PLLA is what produced the large audio
// clicks and horizontal waterfall lines during 10 Hz dial steps.
bool si5351RetuneRxClockSmoothWithProfile(uint32_t frequencyHz,
                                               uint8_t profileTag,
                                               uint8_t ms0Divider) {
  if (!si5351Ready || !rxClockConfigured) return false;
  if ((ms0Divider & 1u) != 0u || ms0Divider < 8) return false;

  // This fast path is valid only while the integer MultiSynth topology is
  // unchanged.  Band/profile changes still use the conservative full rebuild.
  if (currentRxBandIndex != profileTag || currentRxMs0Divider != ms0Divider) {
    return false;
  }

  const double clk0Hz = 4.0 * static_cast<double>(frequencyHz);
  const double pllAHz = clk0Hz * static_cast<double>(ms0Divider);
  if (pllAHz < SI5351_PLL_MIN_HZ || pllAHz > SI5351_PLL_MAX_HZ) return false;

  uint32_t p1, p2, p3;
  si5351CalculateParams(pllAHz / correctedReferenceHz(), p1, p2, p3);

  // One contiguous 8-byte I2C write.  No CLK0 disable, no PLL reset, no RXSW
  // operation and no USB-audio blanking.
  if (!si5351WriteSynth(SI5351_REG_PLLA, p1, p2, p3)) return false;
...
bool si5351RetuneRxClockSmooth(uint32_t frequencyHz) {
  const int bandIndex = bandIndexForFrequency(frequencyHz);
  if (bandIndex < 0) return false;
  const BandPlan &band = BAND_PLANS[bandIndex];
  return si5351RetuneRxClockSmoothWithProfile(
      frequencyHz, static_cast<uint8_t>(bandIndex), band.rxMs0Divider);
}
...
void applyDialFrequency(uint32_t frequencyHz) {
  ...
  // Some CAT clients repeat FA SET commands even when the requested frequency
  // has not changed.  Do not turn a duplicate command into an unnecessary
  // synthesizer update.
  if (frequencyHz == dialFrequencyHz) return;
...
void serviceClockGenerator() {
  ...
  // path, the receive LO sits 10.005 kHz below the displayed/carrier frequency,
  ...
  if (cwTerminalMode) {
    ...
    if (cwRfChanged) {
      ...
      const int desiredBandIndex = bandIndexForFrequency(dialFrequencyHz);

      // Normal CW tuning while receiving must be continuous.  The desired RF
      // stays in-band and identifies the fixed MS0 profile, while the physical
      // QSD LO is 10.005 kHz lower and is allowed to sit just outside a formal
      // amateur-band edge.  For 1/10/100 Hz same-band changes, update only PLLA:
      // no RXSW operation, no CLK0 disable, no PLL reset, no USB-audio blanking.
      bool smoothRxRetuned = false;
      if (!cwSessionNow && cwReceiveRfActive && cwRxLoHz != 0 &&
          desiredBandIndex >= 0 && rxClockConfigured && rxClockEnabled) {
        const BandPlan &band = BAND_PLANS[desiredBandIndex];
        if (currentRxBandIndex == static_cast<uint8_t>(desiredBandIndex) &&
            currentRxMs0Divider == band.rxMs0Divider) {
          smoothRxRetuned = si5351RetuneRxClockSmoothWithProfile(
              cwRxLoHz, static_cast<uint8_t>(desiredBandIndex),
              band.rxMs0Divider);
        }
      }

      if (smoothRxRetuned) {
        // Pre-program the new CW TX carrier while CLK1/drivers remain off.
        // This touches PLLB/MS1 only and does not disturb the continuously
        // running CLK0/QSD receive path.
        txFrequencyReady =
            si5351ConfigureTxClock(static_cast<double>(dialFrequencyHz));
        currentTxFrequencyHz = 0.0;
        rxRetunePending = false;
      } else {
        // Initial CW entry, band/profile changes, or a change during a TX session
        // still use the conservative break-before-make path.
        cwTxEnvelopeHardwareReady = false;
        cwEnvelopeMarkActive = false;
        tinyDxMemoryBarrier();
        si5351EnableClk1(false);
        ...
        bool rxReady = false;
        if (cwRxLoHz != 0 && desiredBandIndex >= 0) {
          const BandPlan &band = BAND_PLANS[desiredBandIndex];
          rxReady = si5351ConfigureRxClockWithDivider(
              cwRxLoHz, band.rxMs0Divider,
              static_cast<uint8_t>(desiredBandIndex));
        }
        const bool qsdRunning = rxReady && si5351EnableClk0(true);

        // TX always uses the actual desired RF/carrier frequency supplied by
        // the app; the 10.005 kHz offset exists only in the receive LO.
        txFrequencyReady =
            si5351ConfigureTxClock(static_cast<double>(dialFrequencyHz));
  ...
  // CAT dial change while already receiving.  Ordinary tuning inside the same
  // amateur band must be continuous: keep CLK0 running, keep RXSW connected,
  // keep USB audio streaming and retain the I/Q/Hilbert history.  Only PLLA's
  // frequency word changes.  This is the path used by 10 Hz tuning steps.
  //
  // A band/profile change still needs a different fixed MS0 divider, so retain
  // the conservative break/blank/rebuild sequence for that exceptional case.
  if (!txModeActive && rxRetunePending) {
    const int targetBandIndex = bandIndexForFrequency(dialFrequencyHz);
    const bool sameRxProfile =
        targetBandIndex >= 0 && rxClockConfigured && rxClockEnabled &&
        currentRxBandIndex == static_cast<uint8_t>(targetBandIndex) &&
        currentRxMs0Divider == BAND_PLANS[targetBandIndex].rxMs0Divider;

    if (sameRxProfile && si5351RetuneRxClockSmooth(dialFrequencyHz)) {
      // Deliberately nothing else: no blanking, no RX switching, no PLL reset,
      // and no DSP reset.  The next ADC/USB frame simply sees the shifted LO.
    } else {
      rxAudioBlankingActive = true;
      rxAudioUnblankAtUs = 0;
      tinyDxMemoryBarrier();
...
void sendCwTerminalStatus() {
  ...
  snprintf(response, sizeof(response),
           "\r\n[CW F=%lu Hz TX=%u WPM SHAPE=5ms RX=USB CENTER=%luHz BUF=%u/%u]\r\n",
           static_cast<unsigned long>(dialFrequencyHz),
           static_cast<unsigned>(cwWpm),
           static_cast<unsigned long>(CW_RX_CENTER_AUDIO_HZ),
...
void sendCwTerminalHelp() {
  cwTerminalWriteRaw(
      "\r\n[CW terminal: text=TX  %F<Hz>=wanted RF/TX carrier  %W<WPM>  %S=status  %Q=exit; RX=USB at F-10005 Hz; BFO/filter/NB/AGC/decoder are app-side]\r\n");
}
...
void enterCwTerminalMode() {
  ...
  setRxSidebandMode(RX_MODE_USB);  // CW RX LO is carrier-10.005 kHz, so wanted CW is on USB.
  cwTerminalMode = true;
...
void executeCwTerminalCommand(const char *command) {
  ...
  if (command[0] == 'C') {
    // Backward-compatible parser for older companion apps.  The second value
    // used to be a radio BFO offset; in the centred-channel architecture it is
    // intentionally ignored because BFO is entirely phone-side.
    uint32_t frequencyHz = 0;
    uint32_t legacyBfoHz = 0;
    if (command[1] == '\0' ||
        !parseUnsignedDecimalPair(command + 1, frequencyHz, legacyBfoHz) ||
        bandIndexForFrequency(frequencyHz) < 0) {
      cwTerminalWriteRaw("\r\n[CW ?C - legacy form is %C<carrierHz>,<ignoredBfo>; use %F<Hz> with current app]\r\n");
      return;
    }
  ...
  if (command[0] == 'F') {
    ...
    if (!parseUnsignedDecimal(command + 1, frequencyHz) ||
        bandIndexForFrequency(frequencyHz) < 0) {
      cwTerminalWriteRaw("\r\n[CW ?F - wanted RF/TX carrier must be inside 40/30/20/17 m]\r\n");
      return;
    }

    // Only the wanted RF is band-limited.  The physical receive oscillator is
    // derived internally at F-10.005 kHz and may sit just outside a band edge.
    applyDialFrequency(frequencyHz);
    sendCwTerminalStatus();
    return;
  }
  ...
  if (command[0] == 'T') {
    // Retained only to give an explicit response to older apps/terminals.
    // Listening pitch is now local to the phone and must not retune the radio.
    cwTerminalWriteRaw("\r\n[CW BFO is app-side only (300..800 Hz); RF unchanged]\r\n");
    return;
  }
